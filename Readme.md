...
COPY --chown=nginx dist/home-ui/browser /usr/share/nginx/html
...
pipeline {
    parameters {
      string(name: 'HOST', description: 'Ingress host', defaultValue: 'ebu.sumerge.local')
      choice(name: 'ENV', choices: ['test'], description: 'Angular environment')
      choice(name: 'NAMESPACE', choices: ['ebu'], description: 'K8s Namespace')
...
sh 'sed -i "s|@DOCKER_REGISTRY@|${DOCKER_REGISTRY}|g" Dockerfile'
                dockerImage = docker.build(registry + "home-ui:$BUILD_NUMBER", "--file=Dockerfile .")
              }
            }
          }
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: home-ui
          image: @DOCKER_REGISTRY@/@NAMESPACE@/home-ui:@BUILD_NUMBER@
